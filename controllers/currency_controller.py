...
class CurrencyController(QObject):
    ...
    def initialize_ui(self):
        ...
        if hasattr(self.model, 'rates') and self.model.rates:
            ...
            self.view.set_status("Appuyez sur 'Mettre à jour les taux' pour charger les taux de change")
    ...
    def convert_currency(self, amount, from_currency, to_currency):
        ...
        if not from_currency or not to_currency:
            self.view.set_status("Veuillez sélectionner des devises valides", is_error=True)
            return
        ...
        try:
            ...
            if rate is None:
                ...
                QTimer.singleShot(1000, lambda: self.retry_conversion(amount, from_currency, to_currency))
                return
            ...
            self.view.set_status(f"Erreur lors de la conversion: {str(e)}", is_error=True)
    ...
    def retry_conversion(self, amount, from_currency, to_currency):
        ...
        if rate is not None:
            result = amount * rate
            self.view.set_conversion_result(result, rate)
            self.view.set_status("Conversion effectuée avec les derniers taux disponibles")
        else:
            self.view.set_status("Impossible d'obtenir le taux de change. Vérifiez votre connexion.", is_error=True)
    ...
    def update_rates(self):
        ...
        try:
            ...
            if not success and not self.model.rates:
                self.view.set_status("Impossible de mettre à jour les taux. Mode hors ligne activé.", is_error=True)
            
        except Exception as e:
            print(f"Erreur lors de la mise à jour des taux: {e}")
            self.view.set_status("Erreur lors de la mise à jour des taux", is_error=True)
    ...
    def try_alternative_api(self):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                self.model.rates_updated.emit(True, "Taux mis à jour avec succès (source alternative)")
                return True
        except Exception as e:
