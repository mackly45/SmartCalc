...
class ScientificView(QWidget):
        ...
        for row, row_buttons in enumerate(buttons):
            for col, text in enumerate(row_buttons):
                ...
                # Style pour les boutons de chiffres
                    ...
                    btn.clicked.connect(
                        lambda checked, t=text: self.on_digit_clicked(t)
                    )
                # Style pour les boutons d'opérations
                elif text in ["+", "-", "*", "/"]:
                    btn.clicked.connect(
                        lambda checked, t=text: self.on_operator_clicked(t)
                    )
    ...
    def calculate_statistics(self):
        ...
        try:
            ...
            QMessageBox.critical(
                self, "Erreur", f"Erreur lors du calcul des statistiques: {str(e)}"
            )
    ...
    def update_history_display(self):
        ...
        if not hasattr(self, "controller") or not hasattr(
            self.controller.model, "history"
        ):
            return
    ...
    def export_history(self):
        ...
        if not hasattr(self, "controller") or not hasattr(
            self.controller.model, "save_history_to_file"
        ):
            return
        ...
        if file_path:
            ...
            if self.controller.model.save_history_to_file(file_path):
                QMessageBox.information(
                    self, "Succès", "L'historique a été exporté avec succès."
                )
            else:
                QMessageBox.warning(
                    self, "Erreur", "Impossible d'exporter l'historique."
                )
