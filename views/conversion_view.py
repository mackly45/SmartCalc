...
class ConversionView(QWidget):
    ...
    convert_requested = pyqtSignal(float, str, str, str)  # value, from_unit, to_unit, conv_type
    ...
    def update_units(self, units):
        ...
        current_from = self.from_unit_combo.currentText() if self.from_unit_combo.count() > 0 else None
        current_to = self.to_unit_combo.currentText() if self.to_unit_combo.count() > 0 else None
    ...
    def update_history(self, history):
        ...
        for entry in history:
            ...
            text = (f"{from_val} {entry['from_unit']} → {to_val} {entry['to_unit']} "
                   f"({entry['type']})")
    ...
    def current_units(self):
        return (self.from_unit_combo.currentText(), 
